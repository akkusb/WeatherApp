...
class JSONModel: NSObject {
    static func toDictionary<T>(model:T) -> [String:Any] where T : Encodable{
        var paramaters : [String:Any] = [:]
        
        do{
            let encoder : JSONEncoder = JSONEncoder()
            let data : Data = try encoder.encode(model)
            if let param = (try JSONSerialization.jsonObject(with: data, options: .mutableContainers)) as? [String:Any] {
                paramaters = param
            }
        }
        catch {
